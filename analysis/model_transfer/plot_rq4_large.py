...
df = pd.read_csv('rq4_f1_large.csv')

palette = hue_pal(0.01, 0.6, 0.65, color_space='hls')
pal = palette(8)
# old_palette = [pal[1], pal[3], pal[0], pal[4], pal[2], pal[5], pal[7]]
def fix(model):
    d = {
        "Transfer-on": "1",
        "Transfer-pc": "2",
...
model_labels = ["SpanBERT (L)", "SpanBERT-On (L)", "Transfer (en)",
                "Transfer (on)", "Transfer (pc)", "XLM-R (L)"]
old_linetype = ["dashed", "dashed", "solid", "solid", "solid", "dashed"]
old_palette = [pal[0], pal[4], pal[2], pal[1], pal[6], pal[5], "pink"]
...
p = (ggplot(new_df, aes(x='layers', y='Average F1', color='model', group='type'))
     + labs(x='Top k layers are trainable', y='Coref Avg. F1', color='model')
...
p.save(filename = 'rq4_f1_large.pdf', height=5, width=5, units = 'in', dpi=400)
