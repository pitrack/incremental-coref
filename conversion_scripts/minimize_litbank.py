...
def get_document(document_lines, tokenizer, language, segment_len):
  ...
  split_into_segments(document_state, segment_len, constraints1, document_state.token_end, sentences=False)
...
def minimize_partition(name, language, extension, labels, stats, tokenizer, seg_len, input_dir, output_dir):
  ...
  output_path = "{}/{}.jsonlines".format(output_dir, name)
  count = 0
...
def minimize_language(language, labels, stats, vocab_file, seg_len, input_dir, output_dir, do_lower_case, model):
  ...
  for name in os.listdir(input_dir):
    minimize_partition(name, language, ".conll", labels, stats, tokenizer, seg_len, input_dir, output_dir)
...
if __name__ == "__main__":
  ...
  if not os.path.isdir(output_dir):
    os.mkdir(output_dir)
  for seg_len in [512]:
    minimize_language("", labels, stats, vocab_file, seg_len, input_dir, output_dir, do_lower_case, model)
